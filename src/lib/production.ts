...
export function validateProduction(input: ProductionHourlyInput) {
  ...
  for (const [key, value] of Object.entries(input)) {
    if (["planQty", "actualQty", "goodQty", "defectQty", "downtimeMinutes"].includes(key) && typeof value === "number" && value < 0) {
  ...
  if (!input.businessDate || !input.shiftId || !input.lineId || !input.modelId || !input.hourStart) {
    errors.push("businessDate, shift, line, model and hour are required");
  }
  return errors;
}
...
export async function createHourlyProduction(input: ProductionHourlyInput) {
  ...
  return addDoc(collection(firestore, "productionHourly"), {
    ...input,
    status: "posted",
...
export async function getProductionRecord(id: string) {
  return getDoc(doc(requireDb(), "productionHourly", id));
}
...
export function productionKpis(records: Array<Partial<ProductionHourlyInput>>) {
  ...
  return {
    ...
    defects,
    downtime,
    planAttainment: plan > 0 ? (actual / plan) * 100 : null,
    yield: actual > 0 ? (good / actual) * 100 : null,
    defectRate: actual > 0 ? (defects / actual) * 100 : null,
  };
}
